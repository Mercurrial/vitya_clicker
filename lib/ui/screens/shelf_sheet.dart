...
///
/// Пропорции задал владелец (docs/DECISIONS.md, «Главный экран»): шапка —
/// около четверти экрана, стена — около 40 %, магазин — остальное; для
/// длинного списка он разворачивается. Шапка берёт свою высоту сама, стена —
/// всё, что осталось, так что считать надо только магазин. Но не меньше двух
/// строк списка: на 320×640 треть экрана — это одна строка.
///
/// Раньше магазин был не меньше половины экрана при 390×844. Сцене тогда
/// оставалось меньше 200 точек, а когда место гостя стало постоянным — меньше
/// 160, и первая банка на полу выходила ростом в сантиметр.
double shelfGarageHeight(double height) =>
    math.max(kShelfMinHeight, height * _kGarageShare);
...
/// Доля экрана под магазином в положении «Гараж».
const double _kGarageShare = 0.35;
